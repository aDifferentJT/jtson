...
class vector_constexpr : private Allocator {
  ...
  public:
    ...
    [[nodiscard]] constexpr auto data() const -> T const * { return _data; }

    [[nodiscard]] constexpr auto size() const -> std::size_t { return _size; }
    [[nodiscard]] constexpr auto capacity() const -> std::size_t { return _capacity; }

    constexpr auto begin()       -> T       * { return _data; }
    [[nodiscard]] constexpr auto begin() const -> T const * { return _data; }

    constexpr auto end()       -> T       * { return _data + _size; }
    [[nodiscard]] constexpr auto end() const -> T const * { return _data + _size; }
    ...
    constexpr vector_constexpr(Allocator allocator = {}) : Allocator{std::move(allocator)} {}

    constexpr vector_constexpr(std::size_t capacity, Allocator allocator = {}) : Allocator{std::move(allocator)}, _data{this->allocate(capacity)}, _capacity{capacity} {}
    ...
    constexpr vector_constexpr(vector_constexpr const & that)
      : vector_constexpr{that.size(), static_cast<Allocator const &>(that)}
    {
      auto it = _data;
      for (auto& x : that) {
        std::construct_at(it++, std::move(x));
    ...
    constexpr vector_constexpr(vector_constexpr&& that) noexcept
    ...
    constexpr auto operator=(vector_constexpr that) -> vector_constexpr& {
      swap(*this, that);
      return *this;
    }
